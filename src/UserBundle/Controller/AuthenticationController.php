...
////        $request_data = $request->request->get('value');  //post
    ...
    public function registerCorporateAction(Request $request) {
        ...
        $conn = $this->get('database_connection');
        $stmt = $conn->prepare('INSERT INTO corporate_account (User_Username,Telephone , Address,District,account_id) VALUES (:userNameR , :telephone, :Address, :district , :account_id );');
        $stmt->bindValue(':userNameR', $userNameR);
    ...
    public function loadHotelPageAction(Request $request) {
        ...
        $conn = $this->get('database_connection');
        $stmt = $conn->prepare('SELECT * FROM corporate_account,user WHERE user.username = corporate_account.User_Username;');
        $stmt->execute();
        $result = $stmt->fetchAll();
